...
public class RecommendAppWithCos {
		...
		for (int i = 0; i < ID_LIST.size(); i++) {
			...
			while (recommendHits.size() < RECOMMEND_NUM) {
				...
				SearchResponse response = client.prepareSearch(INDEX).setTypes(TYPE)
						.setSearchType(SearchType.DFS_QUERY_THEN_FETCH).addFields(FETCH_FIELDS).setFrom(from)
						.setSize(fetchNum).setQuery(getRecQuery(INDEX, TYPE, id, REC_FIELD)).execute().actionGet();
	...
	private static Client createTransportClient(String esHost, int esPort, String cluserName) {
		final Settings settings = ImmutableSettings.settingsBuilder().put("client.transport.sniff", false)
				.put("cluster.name", cluserName).build();

		return new TransportClient(settings).addTransportAddress(new InetSocketTransportAddress(esHost, esPort));
	}

	private static QueryBuilder getMLTQueryForOneIndexedDoc(String index, String type, String id, String field) {
		MoreLikeThisQueryBuilder queryBuilder = QueryBuilders.moreLikeThisQuery(field).docs(new Item(index, type, id))
				.minWordLength(2).minDocFreq(2).minTermFreq(2).maxQueryTerms(100).analyzer("ik_smart");
		return queryBuilder;
	}

	private static QueryBuilder getRecQuery(String index, String type, String id, String field) {
		QueryBuilder mltQuery = getMLTQueryForOneIndexedDoc(index, type, id, field);
		BoolFilterBuilder notIdFilter = FilterBuilders.boolFilter().mustNot(FilterBuilders.idsFilter(type).addIds(id));
		return QueryBuilders.filteredQuery(mltQuery, notIdFilter);
	}

	private static boolean checkDuplicated(SearchHit hit, List<SearchHit> hitList) {
		try {
			...
			for (int i = 0; i < hitList.size(); i++) {
				...
				String originUrl2 = hit2.getFields().get("origin_url").getValue();
